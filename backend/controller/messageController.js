...
const { getRecieverSocketId, io } = require('../socket/socket1');
...
exports.sendMessage=async(req,resp)=>{
    try{ 
        ...
        let gotConversation=await Conversation.findOne({particapants:{$all:[senderId,recieverId]}});
        if(!gotConversation){
        ...
        // console.log("MESAGE",newMessage,"Conversation",gotConversation);
...
exports.getMessage=async(req,resp)=>{
    try{
        ...
        const allMessages=await Conversation.findOne({particapants:{$all:[senderId,recieverId]}}).populate("messages");
